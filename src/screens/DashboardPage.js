...
const StatCard = ({ title, children, bgColor = "#ffffff" }) => (
    <Card
        className="h-100 shadow-sm"
        style={{ backgroundColor: bgColor, transition: "transform 0.2s ease", cursor: "pointer", border: "1px solid #cbd5e1" }}
        onMouseEnter={(e) => e.currentTarget.style.transform = "scale(1.05)"}
        onMouseLeave={(e) => e.currentTarget.style.transform = "scale(1)"}
    >
        <Card.Body className="d-flex flex-column align-items-center justify-content-center py-5 px-2">
            <div style={{ fontSize: "13px", color: "#6b7280", fontWeight: "600", textTransform: "uppercase", letterSpacing: "0.5px", textAlign: "center", lineHeight: "1.2", marginBottom: "4px" }}>{title}</div>
            {children}
        </Card.Body>
    </Card>
);
...
const ChartCard = ({ title, action, children, updatedAt }) => (
    <Card
        className="h-100 shadow-sm"
        style={{ borderRadius: "0px", backgroundColor: "#ecf4f8ff", transition: "all 0.25s ease", cursor: "pointer", border: "1px solid #cbd5e1" }}
        onMouseEnter={(e) => {
...
const CapacityComparisonChart = ({ partial, available, partialCount, availableCount }) => {
    const data = [
        { label: "Partially Available", value: partial, color: "#D97706", count: partialCount },
        { label: "Fully Available", value: available, color: "#059669", count: availableCount }
    ];
...
const HorizontalBarChart = ({ data, color = "#4F46E5" }) => {
    ...
    return (
        <div className="min-vh-100 bg-light">
            ...
            <Container fluid="lg" className="py-4">
                ...
                <Row className="g-3 mb-4" xs={1} md={5}>
                    <Col>
                        <StatCard title="Total Employees" bgColor="#F0FDFA">
                            <div style={{ fontSize: "28px", fontWeight: "700", color: "#111827", marginTop: "4px" }}>{metrics.totalPeople || 0}</div>
                        </StatCard>
                    </Col>
                    <Col>
                        <StatCard title="Projects" bgColor="#F0FDFA">
                            <div className="d-flex gap-3 mt-2">
                                <div className="text-center">
                                    <div style={{ fontSize: "18px", fontWeight: "700", color: "#059669" }}>{metrics.ongoingProjectsCount || 0}</div>
                                    <div style={{ fontSize: "10px", color: "#6b7280" }}>Ongoing</div>
                                </div>
                                <div style={{ width: "1px", backgroundColor: "#e5e7eb" }}></div>
                                <div className="text-center">
                                    <div style={{ fontSize: "18px", fontWeight: "700", color: "#3b82f6" }}>{metrics.openProjectsCount || 0}</div>
                                    <div style={{ fontSize: "10px", color: "#6b7280" }}>Open</div>
                                </div>
                            </div>
                        </StatCard>
                    </Col>
                    <Col>
                        <StatCard title="No of Employees Available" bgColor="#F0FDFA">
                            <div style={{ fontSize: "28px", fontWeight: "700", color: "#22c55e", marginTop: "4px" }}>{metrics.availableEmployeeCount || 0}</div>
                        </StatCard>
                    </Col>
                    <Col>
                        <StatCard title="Employees Partially Available" bgColor="#F0FDFA">
                            <div style={{ fontSize: "28px", fontWeight: "700", color: "#f59e0b", marginTop: "4px" }}>{metrics.partialEmployeeCount || 0}</div>
                        </StatCard>
                    </Col>
                    <Col>
                        <StatCard title="No of Employees Occupied" bgColor="#F0FDFA">
                            <div style={{ fontSize: "28px", fontWeight: "700", color: "#ef4444", marginTop: "4px" }}>{metrics.occupiedEmployeeCount || 0}</div>
                        </StatCard>
                    </Col>
                </Row>
